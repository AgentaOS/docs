...
Start accepting payments in minutes. Everything here runs in **test mode**, a full test environment with test money, so there is nothing to verify and nothing to lose.

Choose the path that fits how you build:
...
app.post('/webhooks', express.raw({ type: 'application/json' }), (req, res) => {
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      // stablecoin checkouts, they are null for card and bank transfers.
    }
