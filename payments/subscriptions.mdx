...
<Steps>
  ...
  <Step title="Share checkoutUrl">
    The buyer opens `product.checkoutUrl` and pays with a card at the hosted checkout, exactly like a one-time payment. Logged-in users: create a checkout with `checkouts.create({ linkId: product.id })` instead. See [Subscribe Users in Your App](/guides/saas-subscriptions).
  </Step>
...
<CardGroup cols={2}>
  <Card title="Subscribe users in your app" icon="user-check" href="/guides/saas-subscriptions">
    Logged-in users: checkout per profile, then status, invoices, cancel.
  </Card>
