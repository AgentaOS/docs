...
<AccordionGroup>
  <Accordion title="checkout.session.completed">
    Fired when a checkout's payment is confirmed, on any rail (card, SEPA, stablecoin). Resolved via the checkout's own webhook, then its payment link's, then your org default.
  </Accordion>
