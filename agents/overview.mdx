...
Every checkout returns an `x402Url` next to the human `checkoutUrl`. Your agent can **discover, price, and pay** for pay-per-call APIs on its own, over the [x402 protocol](/sub/x402), no card and no human in the loop. Machine-to-machine commerce, built in.
...
| Tool | What it does |
|---|---|
| `agenta_pay_create_checkout` | Create a checkout. Returns a `checkoutUrl` (human) and an `x402Url` (agent payer). |
...
<CardGroup cols={2}>
  ...
  <Card title="Agent-to-agent payments" icon="robot" href="/sub/x402">
    Let agents pay for 402-protected resources automatically.
  </Card>
