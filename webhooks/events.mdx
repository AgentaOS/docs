...
Fired when a checkout is paid, by card, bank transfer (SEPA), or stablecoin. This is the event to listen for to fulfill an order.
...
<Note>
`tx_hash`, `payer`, and `network` describe on-chain settlement. For checkouts paid by card or bank transfer (SEPA), these fields describe the settlement rail rather than a blockchain and may be empty, use `vendor_reference` for the off-chain reference and don't assume the on-chain fields are always populated. Key fulfillment off `session_id` and `amount`/`currency`, which are present for every payment method.
</Note>
