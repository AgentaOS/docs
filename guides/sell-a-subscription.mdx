...
<Steps>
  <Step title="Create a subscription payment link">
    ...
    <Tabs>
      <Tab title="SDK">
        ...
        console.log(plan.id);          // linkId — pass this to checkouts.create
        console.log(plan.checkoutUrl); // buyer URL — /pay/… slug, not the API id
        ```
      </Tab>
      <Tab title="cURL">
        ```bash
  ...
  <Step title="Share the buyer link, or check out a known user">
    Unknown buyer: share `plan.checkoutUrl`.

    Logged-in user in your app: create a checkout from `plan.id` (`linkId`) and pass your user id in metadata. Full flow: [Subscribe users in your app](/guides/saas-subscriptions).
