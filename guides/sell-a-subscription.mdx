...
By the end of this guide you'll have a subscription product buyers can pay into, a real test subscriber, and you'll know how to list who's subscribed and cancel someone who asks. This runs in **test mode**, so the "buyer" is you, paying with the test card.
...
<Steps>
  <Step title="Create a subscription product">
    Set `type: 'subscription'` and a `billingInterval`. Every other field works the same as a one-time product.
    ...
    <Tabs>
      <Tab title="SDK">
        ...
        console.log(product.id);          // linkId — pass this to checkouts.create
        console.log(product.checkoutUrl); // buyer URL — /pay/… slug, not the API id
        ```
      </Tab>
      <Tab title="cURL">
        ```bash
    ...
    <Note>
    There's no CLI command for products yet, one-time or subscription. `agenta pay checkout` only creates single, non-recurring checkouts. Create a subscription product from the SDK, the REST API, or the dashboard's **Catalog → Products** page.
    </Note>
  </Step>

  <Step title="Share the buyer link, or check out a known user">
    Unknown buyer: share `product.checkoutUrl`.

    Logged-in user in your app: create a checkout from `product.id` (`linkId`) and pass your user id in metadata. Full flow: [Subscribe Users in Your App](/guides/saas-subscriptions).

    Test card at the hosted checkout:
  ...
  <Step title="Confirm the subscription is active">
    <Tabs>
      <Tab title="SDK">
        ...
        const sub = page.items.find((s) => s.planName === 'Pro, monthly');
